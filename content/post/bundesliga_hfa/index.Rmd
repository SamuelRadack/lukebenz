...
bund <- filter(x, league == "German Bundesliga") %>%
  filter(date <= Sys.Date(), date >= "2020-05-16" ) %>%
...
get_exp_home_points <- function(x) {
  case_when(runif(nrow(bund)) <= bund$prob1 ~ 3,
            runif(nrow(bund)) <= bund$prob1  + bund$probtie ~ 1,
            T ~ 0) %>%
    sum()
}
